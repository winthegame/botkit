...
function Slackbot(configuration) {
    ...
    slack_botkit.configureSlackApp = function(slack_app_config, cb) {
        ...
        slack_botkit.log("HERE IS THE APP: " + JSON.stringify(slack_app_config));
        if (!slack_app_config || !slack_app_config.clientId ||
            ...
            if (slack_app_config.urlPrefix)
                slack_botkit.config.urlPrefix = slack_app_config.urlPrefix;
            else
                slack_botkit.config.urlPrefix = "";
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        slack_botkit.log("config: " + JSON.stringify(slack_botkit.config));
        ...
        webserver.get(slack_botkit.config.urlPrefix + '/login', function(req, res) {
            res.redirect(slack_botkit.getAuthorizeURL());
        });

        slack_botkit.log('** Serving oauth return endpoint: http://MY_HOST:' + slack_botkit.config.port + slack_botkit.config.urlPrefix + '/oauth');

        webserver.get(slack_botkit.config.urlPrefix + '/oauth', function(req, res) {
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                if (!team) {
                                    ...
                                    team = {
                                        ...
                                        url: identity.url,
                                        name: identity.team,
                                        appId: slack_botkit.config.appId
                                    };
                                }
                                ...
                                if (auth.bot) {
                                    ...
                                    console.log("Updating the team.bot config/auth data!");

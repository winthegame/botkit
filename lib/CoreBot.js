...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        /**
         * Reacts to the user's most recent messsage.
         * For documentation on the API call see https://api.slack.com/methods/reactions.add
         *
         * @param {string} reaction the reaction to react with. For example, "+1".
         * @author @rafaelcosman
         */
        this.react = function(reaction) {
            bot.api.reactions.add({
                timestamp: this.transcript[this.transcript.length - 1].ts,
    ...
    botkit.hears = function(keywords, events, cb, continue_propagation) {
    ...
    botkit.hearsIntent = function(intent_names, confidence, events, cb) {
        if (!botkit.witbot) {
            botkit.log('Please set token_wit in controller.spawn()');
            return;
        }

        if (typeof(intent_names) == 'string') {
            intent_names = intent_names.split(/\,/g);
        }

        // This is based off of https://www.npmjs.com/package/witbot#example
        botkit.hears('.*', events, function(bot, message) {
            var wit = botkit.witbot.process(message.text, bot, message);

            for (index in intent_names) {
                wit.hears(intent_names[index], confidence, cb);
            }
        }, true);
    ...
    botkit.spawn = function(config, cb) {
        ...
        if (config.token_wit) {
            ...
            botkit.witbot = Witbot(config.token_wit);
        }
